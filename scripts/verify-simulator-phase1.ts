...
// --- 시나리오 1: 조건 불충족 효과 미발동 (노지코를 나미 외 리더로 소환 시) ---
{
  ...
  console.assert(
    state.players.p2.zones.secondary.length === 1,
    "FAIL: 상대 캐릭터가 패로 되돌아갔습니다.",
  );
...
// --- 시나리오 2: '상대 1장 레스트'가 정확히 1장만 레스트 (이조) ---
{
  ...
  console.assert(
    p2_c1?.rested === true,
    "FAIL: 파워가 더 높은 캐릭터(p2-c1)가 레스트되지 않았습니다.",
  );
...
// --- 시나리오 3: don_rest 4 미만 보유 시 기동 효과 발동 불가, 4 이상 시 don 차감 (루피 리더) ---
{
  ...
  const state1 = optcgEngine.applyAction(state, { type: "activate_main", sourceIid: "p1-leader" });
  ...
  console.assert(
    getBattlePower(state1.players.p1.zones.primary[0]) === 5000,
    "FAIL: don 부족에도 루피 파워가 올라갔습니다.",
  );

  // 2) donActive가 4일 때
  state.players.p1.donActive = 4;
  const state2 = optcgEngine.applyAction(state, { type: "activate_main", sourceIid: "p1-leader" });
  ...
  console.assert(
    getBattlePower(state2.players.p1.zones.primary[0]) === 6000,
    "FAIL: 루피 파워가 +1000 되지 않았습니다.",
  );
...
// --- 시나리오 4: this_turn 디버프가 다음 턴 refresh 후 원복 (오타마) ---
{
  ...
  console.assert(
    getBattlePower(cBefore) === 1000,
    "FAIL: 오타마에 의한 파워 디버프가 적용되지 않았습니다.",
  );
  ...
  console.assert(
    getBattlePower(targetUnit) === 3000,
    "FAIL: 턴 교대 후 오타마 디버프가 해제되지 않았습니다.",
  );
...
// --- 시나리오 5: 카운터 이벤트 +4000이 그 전투에만 반영되고 종료 후 흔적 없음 (거미집 그물) ---
{
  ...
  // p1 리더로 p2 리더 공격
  state = optcgEngine.applyAction(state, {
    type: "attack",
    attackerIid: "p1-leader",
    targetIid: "p2-leader",
  });
  console.assert(
    state.pendingResponse !== null,
    "FAIL: 공격 선언 후 카운터 윈도우가 열리지 않았습니다.",
  );
  ...
  console.assert(
    state.pendingResponse === null,
    "FAIL: 전투 해결 후 카운터 윈도우가 닫히지 않았습니다.",
  );
  console.assert(
    getBattlePower(state.players.p2.zones.primary[0]) === 5000,
    "FAIL: 전투 완료 후 나미 리더 파워가 원복되지 않았습니다.",
  );
  console.assert(
    state.players.p2.zones.graveyard.some((c) => c.code === "E-NET"),
    "FAIL: 사용한 거미집 그물이 트래시로 이동하지 않았습니다.",
  );
...
// --- 시나리오 6: 이벤트 사용 시 don 지불·트래시 이동·KO 적용 (항마의 상) ---
{
  ...
  console.assert(
    state.players.p1.donActive === 0,
    "FAIL: 이벤트 플레이 비용 donActive가 차감되지 않았습니다.",
  );
  console.assert(
    state.players.p1.donRested === 1,
    "FAIL: 이벤트 플레이 비용 donRested가 누적되지 않았습니다.",
  );

  // 2) 효과 KO 적용 확인 (상대 캐릭터 KO되어 graveyard로 이동)
  console.assert(
    state.players.p2.zones.secondary.length === 0,
    "FAIL: 상대 캐릭터가 KO되지 않았습니다.",
  );
  console.assert(
    state.players.p2.zones.graveyard.some((c) => c.iid === "p2-c1"),
    "FAIL: KO된 상대 캐릭터가 트래시로 이동하지 않았습니다.",
  );

  // 3) 이벤트 카드 트래시 이동 확인
  console.assert(
    state.players.p1.zones.hand.length === 0,
    "FAIL: 사용한 이벤트 카드가 손패에 남아있습니다.",
  );
  console.assert(
    state.players.p1.zones.graveyard.some((c) => c.code === "E-KOMA"),
