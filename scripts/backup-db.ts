...
=============================================================${colors.reset}
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    console.error(
      `${colors.red}✖ 오류: Supabase 접속을 위한 환경변수가 설정되지 않았습니다.${colors.reset}`,
    );
    console.error(
      `${colors.yellow}💡 해결 방법: 프로젝트 루트의 .env 파일에 SUPABASE_URL 및 SUPABASE_SERVICE_ROLE_KEY를 정의해 주세요.${colors.reset}\n`,
    );
    process.exit(1);
  }
  ...
  if (!isUsingServiceRole) {
    console.warn(
      `${colors.yellow}⚠️ 경고: SUPABASE_SERVICE_ROLE_KEY가 감지되지 않아 퍼블릭 API Key를 사용합니다.${colors.reset}`,
    );
    console.warn(
      `${colors.yellow}일부 비공개 테이블(RLS 적용 테이블)은 백업되지 않을 수 있습니다.${colors.reset}\n`,
    );
  }
  ...
  try {
    for (const table of tables) {
      ...
      if (error) {
        console.error(
          `     ${colors.red}✖ ${table.name} 백업 실패: ${error.message}${colors.reset}`,
        );
        continue;
      }
      ...
      if (table.hasPII && processedData.length > 0) {
        console.log(
          `     ${colors.yellow}🛡️  ${table.name} 내 개인정보(PII) 감지됨. 즉시 익명화 필터링 적용 중...${colors.reset}`,
        );
        ...
        console.log(
          `     ${colors.green}✓ ${table.name} ${processedData.length}건 마스킹 및 수집 완료.${colors.reset}`,
        );
      } else {
        console.log(
          `     ${colors.green}✓ ${table.name} ${processedData.length}건 수집 완료 (마스킹 불필요).${colors.reset}`,
        );
      }
