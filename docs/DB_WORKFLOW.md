...
> **전환 공지(2026-06-09):** 앞으로 Lovable에는 DB 변경을 위임하지 않습니다. 이 문서의 Lovable 중심 절차는 기존 이력 확인용이며, 새 DB 변경은 마이그레이션 SQL/검증 절차를 남긴 뒤 새 Supabase 환경에서 적용하는 방향으로 개정합니다. 상세 전환 항목은 `docs/LOVABLE_HANDOFF.md`, 신규 작업 흐름은 `docs/CODEX_WORKFLOW.md`를 기준으로 합니다.
>
> 본 문서는 DuelNight 프로젝트에서 **데이터베이스(Supabase) 변경**을 다룰 때
> 과거 세 도구(Lovable / Claude / Antigravity)가 어떻게 역할을 나누고 동기화했는지 보관합니다.
> 아래 Lovable 중심 절차는 신규 작업에 적용하지 않습니다.

---

## 0. 기존 한 줄 요약 (보관용)

아래 내용은 Lovable을 사용하던 시기의 역할 분담입니다. **신규 작업에는 적용하지 않습니다.**

- **과거 기준:** DB(스키마·데이터·RLS·함수·트리거)는 Lovable이 단독으로 관리·적용
- **과거 기준:** Claude / Antigravity는 "적용할 SQL/마이그레이션을 작성해 사용자에게 전달"만 함
- **유지 기준:** 앱 코드(프론트엔드, API 라우트, 컴포넌트, i18n 등)는 GitHub 커밋·PR 기준으로 관리

> 현재 전환 방향은 **DB 변경을 Lovable에 위임하지 않고**, 새 Supabase 환경에서 검증 가능한 마이그레이션 절차로 적용하는 것입니다.
