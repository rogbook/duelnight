...
| ✅ 적재 완료 (본대 6장) | 이조(등장시 레스트)·오타마(등장시 -2000)·항마의 상(메인 KO)·노지코(조건부 바운스)·거미집 그물(카운터+트리거)·루피 리더(기동) | `20260613110000` — `scripts/build-effect-migration.ts`로 zod 검증 후 생성. 메커니즘은 `verify-simulator-phase1.ts` 6시나리오로 증명됨 |
| ⏸ 보류 (Phase 2+) | 지옥의 심판·화이트 스네이크(상대약화 카운터·덱조작 — 미검증 메커니즘), 리쿠/잠바이 블로커 외 부가효과, 조로·오뎅·로(passive·미지원 action), 고든·후즈후·Mr.1(search/optional cost), 우솝·키드 부가(on_event), 스테이지 2장, 미스 올 선데이 | 인터프리터 추가 보강 필요 |

> 데이터 정정: 오타마(OP01-006) cost 111→1 오타 수정(`20260613110500`). 효과 추가는 `scripts/build-effect-migration.ts`의 `EFFECTS`에 항목을 더하고 재실행 → 검증 통과분만 마이그레이션 생성.
