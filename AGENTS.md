...
> - 방향이 모호하거나 다른 영역에 영향이 갈 때는 임의 진행하지 말고 **[docs/QUESTIONS.md](./docs/QUESTIONS.md)에 질문을 적고 push**한다. Claude가 같은 파일에 답변을 push하면 pull 받아 진행한다. (질문을 남겼으면 답변 전까지 해당 부분은 작업 보류 — 다른 부분은 진행 가능)
...
7. **모든 계획서·보고·질문은 한국어로 작성한다.**
8. 의미 있는 작업은 `docs/`에 기록 + 아래 **작업 전달 형식**으로 마무리 보고:
...
## 2-1. 하네스 설계 원칙 (모든 도구 공통 자세)

> 출처: 하네스 설계 강의(`docs/HARNESS_PRINCIPLES.md` 요약). Karpathy 4원칙([CLAUDE.md](./CLAUDE.md) §1-1)과 함께 적용한다.
> 핵심 명제: **에이전트 성능 = 모델 × 하네스.** 같은 모델이라도 아래 구조로 결과 품질이 갈린다.

| 원칙               | 우리 프로젝트 적용                                                                                                                                                                              |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **지식 맵 점진 공개** | 이 문서·CLAUDE.md는 "안내판", 상세는 `docs/`에 둔다. 한 파일에 다 넣지 말고 작업에 필요한 문서만 그때 열어 컨텍스트를 아낀다.                                                                    |
| **검증을 루프에 내장** | "느낌으로 개선하지 않는다." 기능마다 재현·검증 수단(verify 스크립트·zod 게이트·테스트)을 **먼저** 만들고 통과를 커밋 게이트로 삼는다. 예: `scripts/verify-simulator-phase1.ts`, `scripts/build-effect-migration.ts`. |
| **교차(2중) 검증**    | 만든 주체와 검증 주체를 분리한다(부 개발자 구현 → Claude 검수·병합). §1 역할 분담의 근거.                                                                                                       |
| **단순한 구성부터**   | 큰 프레임워크 선제 도입 금지. 작은 성공 후 필요한 만큼만. 컨텍스트는 길이보다 high-signal. (CLAUDE.md §1과 동일)                                                                                |
| **가정 정기 재검토**  | 하네스 각 요소는 모델 한계를 가정한 우회다. 모델이 좋아지면 불필요해질 수 있으니 주기적으로 점검·제거한다.                                                                                       |

**하네스 리스크(경계할 것):** ① 승인 피로 — 반복 승인 시 검토 소홀, 무거운 명령 남발 금지 ② 코드 드리프트 — 복붙 증가·아키텍처 이탈, 최소 diff·기존 패턴 재사용으로 방지 ③ 도구 오염 — 신뢰 못 할 MCP/도구 사용 주의.
...
| ~~게이머 UI 2단계(카드·덱·전적 톤 맞춤)~~                           | Claude (Antigravity 한도로 인계)  | ✅ **완료 (2026-06-13)** — 4개 파일 game 토큰 전환, 카드 색상칩(적/녹)은 의미색 유지. 두 테마×3화면 검증 |
