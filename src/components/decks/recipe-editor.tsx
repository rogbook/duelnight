...
export function RecipeEditor({ deck }: { deck: Deck }) {
  ...
  const cardMap = useMemo(() => ({ get: (code: string) => cardMapData[code] }), [cardMapData]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-md border border-border bg-muted/20 p-3">
        <div className="flex items-center justify-between text-xs font-medium">
          <div className="flex items-center gap-2">
            ...
            <span className={totalCards > maxTotal ? "text-destructive font-bold" : ""}>
              {totalCards}
            </span>
            <span className="text-muted-foreground">/ {maxTotal}</span>
          </div>
          {game === "dtcg" && <span>디지타마 {digitamaCount} / 5</span>}
          {game === "ptcg" && hasAceInDeck && (
            <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full font-bold border border-blue-200">
              ACE SPEC
            </span>
          )}
        </div>
        ...
        <ul className="mt-3 space-y-1.5 max-h-48 overflow-y-auto pr-1">
          {deckCards.map((dc) => {
            ...
            return (
              <li
                key={dc.id}
                className="flex items-center gap-2 text-xs bg-card p-1.5 rounded border border-border"
              >
                {card?.image_url ? (
                  <button
                    type="button"
                    className="h-8 w-6 shrink-0 overflow-hidden rounded"
                    onClick={() => openZoom(card.image_url, card.name ?? dc.card_code)}
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="h-8 w-8 inline-flex items-center justify-center hover:bg-muted rounded touch-manipulation"
                    aria-label="수량 감소"
                  >
                    −
                  </button>
                  ...
                  <button
                    onClick={() => removeCard(dc.id)}
                    className="h-8 w-8 ml-1 inline-flex items-center justify-center text-muted-foreground hover:text-destructive touch-manipulation"
                    aria-label="카드 제거"
                  >
      ...
      <div className="space-y-2.5">
        ...
        <div className="flex flex-wrap gap-1.5">
          <Select value={filterType} onValueChange={setFilterType}>
            <SelectTrigger className="h-8 text-[11px] w-auto min-w-[85px]">
          ...
          {game === "dtcg" && (
            <Select value={filterLevel} onValueChange={setFilterLevel}>
              <SelectTrigger className="h-8 text-[11px] w-auto min-w-[70px]">
                <SelectValue placeholder="레벨" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Lv.전체</SelectItem>
                {DIGIMON_LEVELS.map((lv) => (
      ...
      <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1">
        {isFetching && (
          <p className="text-xs text-center py-6 text-muted-foreground">카드를 찾는 중...</p>
        )}
        {!isFetching && groupedResults.length === 0 && (
          <p className="text-xs text-center py-6 text-muted-foreground">검색 결과가 없습니다.</p>
        )}
        ...
        {groupedResults.map((versions) => {
          ...
          const isLeaderColor =
            game === "optcg" && deck.colors?.some((c) => main.colors?.includes(c));

          return (
            <div
              key={main.name}
              className={`rounded-xl border p-3.5 space-y-3.5 transition-colors ${isLeaderColor ? "border-primary/40 bg-primary/5 shadow-sm" : "border-border bg-card"}`}
            >
              <div className="flex gap-4">
                <button
                  onClick={() => openZoom(main.image_url, main.name)}
                  className="relative shrink-0 group self-start"
                >
                  <img
                    src={main.image_url ?? ""}
                    className="h-24 w-16 rounded-md object-cover shadow-md"
                  />
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2.5 space-y-1">
                    ...
                    {main.effect && (
                      <p className="text-[10px] text-muted-foreground line-clamp-2 leading-relaxed mt-1 italic">
                        {main.effect}
                      </p>
                    )}
                  </div>
              ...
              <div className="grid grid-cols-1 gap-1.5">
                {versions.map((v) => (
                  <div
                    key={v.code}
                    className="flex items-center justify-between bg-muted/30 rounded-lg px-3 py-2 text-[11px] border border-transparent hover:border-border transition-all"
                  >
                    ...
                    <div className="flex items-center gap-3">
                      {getQty(v.code) > 0 && (
                        <span className="font-black text-primary text-xs">× {getQty(v.code)}</span>
                      )}
                      <button
                        onClick={() => addCard(v)}
                        className="bg-foreground text-background px-3 py-1.5 rounded-md text-[10px] font-bold hover:scale-105 active:scale-95 transition-transform"
                      >
                        추가하기
                      </button>
                    </div>
