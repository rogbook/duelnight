...
async function parseFile(
  f: File,
): Promise<{ rows: CardRow[]; errors: { line: number; reason: string }[] }> {
...
const SAMPLE_ROWS = [
  {
    ...
    레어도: "L",
    효과: "[활성 메인] 효과 텍스트 예시",
    이미지: "https://example.com/op01-001.png",
  },
  {
    코드: "OP01-002",
    세트: "OP01",
...
export function CardUploader({ isAdmin, onComplete }: Props) {
  ...
  const onUploadImages = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const original of files) {
        ...
        if (upErr) {
          toast.error(`${original.name}: ${upErr.message}`);
          done++;
          continue;
        }
      ...
      setDupChecked(false);
      toast.success(
        `이미지 ${newRows.length}장 업로드 (WebP 변환 완료). "이름 비어있는 행 OCR"로 자동 채울 수 있어요.`,
      );
    } finally {
      setBusy(false);
    }
  ...
  const ocrRow = async (idx: number): Promise<boolean> => {
    ...
    try {
      ...
      if (Array.isArray(d.colors) && d.colors.length && r.colors.length === 0)
        patch.colors = (d.colors as unknown[]).map(String);
  ...
  const checkDuplicatesAgainstDb = async () => {
    const codes = Array.from(
      new Set(rows.map((r) => (r.code || "").trim().toUpperCase()).filter(Boolean)),
    );
    if (codes.length === 0) {
      toast.error("코드가 입력된 행이 없습니다");
      return;
    }
    setBusy(true);
    try {
  ...
  const submit = async () => {
    ...
    try {
      ...
      for (let i = 0; i < cardRows.length; i += CHUNK) {
        ...
        const isExtraColErr = (msg?: string) =>
          !!msg && /extra|schema cache|column .* does not exist/i.test(msg);
        const stripExtra = (arr: typeof slice) => arr.map(({ extra: _drop, ...x }) => x);
        if (isAdmin) {
          let { error } = await supabase
            .from("cards")
            .upsert(slice as never, { onConflict: "code" });
          if (isExtraColErr(error?.message)) {
            ({ error } = await supabase
              .from("cards")
              .upsert(stripExtra(slice) as never, { onConflict: "code" }));
          }
          if (error) throw error;
      ...
      if (illustAdded)
        parts.push(
          isAdmin
            ? `${illustAdded}장 추가 일러스트 등록`
            : `${illustAdded}장 추가 일러스트 검수 대기`,
        );
  ...
  const importDriveFiles = async () => {
    ...
    try {
      ...
      setRows((prev) => [...prev, ...newRows]);
      toast.success(
        `${newRows.length}건 가져오기 완료. "이름 비어있는 행 OCR"로 정보를 채워보세요.`,
      );
      setDriveFiles([]);
      setDriveFolderUrl("");
    } catch (e) {
  ...
  const importCardToRow = (c: Record<string, unknown>): CardRow => {
    ...
    if (importGame === "dtcg") {
      ...
      if (e.category && DIGIMON_CATEGORY_TYPE[e.category])
        row.type = DIGIMON_CATEGORY_TYPE[e.category];
  ...
  const importFromUrl = async () => {
    ...
    try {
      ...
      setImportUrl("");
      toast.success(
        `${collected.length}건 가져옴 (${lastPage}페이지) — 아래 표에서 검수 후 등록하세요. (출처 표기 필요)`,
      );
    } catch (e) {
      toast.error("가져오기 실패: " + (e as Error).message);
    } finally {
    ...
    <div className="space-y-4">
      <Tabs defaultValue="single">
        <TabsList className="grid w-full grid-cols-3 md:grid-cols-5">
          <TabsTrigger value="single">
            <Plus className="mr-1 h-4 w-4" />한 장씩 입력
          </TabsTrigger>
        ...
        <TabsContent value="url">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                공식 카드 페이지 주소를 넣으면 카드 정보를 자동 추출해 아래 표에 채웁니다. 지원:
                디지몬(digimoncard.co.kr)·원피스 등 공식 카드 페이지. <b>개별 카드 상세 페이지</b>{" "}
                URL이 가장 정확합니다. 이미지는 원본 링크를 그대로 참조(핫링크)하며 재호스팅하지
                않습니다 — <b>출처 표기가 필요</b>합니다. 일부 사이트는 봇 차단(403)이나 핫링크
                차단으로 자동 수집/이미지 표시가 안 될 수 있습니다.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="flex flex-col gap-2 sm:flex-row">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <Input
                    ...
                    value={importPages}
                    onChange={(e) =>
                      setImportPages(Math.max(1, Math.min(50, Number(e.target.value) || 1)))
                    }
                    disabled={importing}
                    className="w-16"
                  />
              ...
              <p className="text-xs text-muted-foreground">
                <b>목록 페이지</b>를 여러 장 한 번에 가져오려면 "페이지"를 2 이상으로
                설정하세요(목록 URL에 page 파라미터를 붙여 순회, 중복 코드 자동 제거). 개별 카드
                상세 URL이면 1로 두세요.
              </p>
              <p className="text-xs text-muted-foreground">
                ⚠️ 동적(JS)으로 로딩되는 페이지는 자동 추출이 어려울 수 있고, 봇 차단(403)·핫링크
                차단 사이트도 있습니다. 추출 결과는 AI 기반이라 부정확할 수 있으니 등록 전 반드시
                검수하세요.
              </p>
            </CardContent>
          </Card>
        ...
        <TabsContent value="single">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                입력하면 아래 표에 행이 추가됩니다. 여러 장 모은 뒤 한 번에 등록하세요.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="excel">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                한국어 헤더를 자동 인식합니다: 코드, 세트, 게임, 이름, 종류, 색상, 비용, 파워,
                카운터, 속성, 레어도, 효과, 이미지. 이미지 칸에는 일반 URL 또는{" "}
                <b>구글 드라이브 공유 링크</b>를 넣을 수 있습니다 (자동 변환, 공개 권한 필요).
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                샘플 파일에는 <b>카드샘플</b> 시트(예시 3건)와 <b>필드설명</b> 시트(필드명·필수
                여부·설명·예시)가 함께 들어 있어요.
              </p>
        ...
        <TabsContent value="images">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                여러 이미지를 한번에 업로드하면 파일명에서 카드 코드를 자동 추출합니다 (예:{" "}
                <code>OP01-001.png</code> → 코드 <code>OP01-001</code>). 나머지 정보는 아래 표에서
                채워주세요.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <Label htmlFor="all-drive" className="text-xs">
                            전체 선택
                          </Label>
                        </div>
                      </div>
                      ...
                      <Button
                        ...
                        disabled={busy || selectedDriveFiles.size === 0}
                      >
                        {busy
                          ? `가져오는 중… (${progress.done}/${progress.total})`
                          : `선택한 ${selectedDriveFiles.size}개 이미지 가져오기`}
                      </Button>
                    </div>
                  )}
                </div>
      ...
      {errors.length > 0 && (
        <Card className="border-destructive/40">
          <CardHeader>
            <CardTitle className="text-sm text-destructive">파일 오류 {errors.length}건</CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <CardHeader className="flex flex-col gap-3">
            <div className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  ...
                  {internalDups.size > 0 && (
                    <span className="text-amber-600"> · 내부 중복 코드 {internalDups.size}종</span>
                  )}
                  {dupChecked && <span className="text-amber-600"> · DB 중복 {dbDupCount}건</span>}
                  <span className="text-muted-foreground">
                    {" "}
                    · 자동 임시저장됨 (Ctrl+Enter로 등록)
                  </span>
                </CardDescription>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" size="sm" onClick={applyAutoFix}>
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={checkDuplicatesAgainstDb}
                  disabled={busy}
                >
                ...
                <Button variant="ghost" size="sm" onClick={addBlank}>
                  <Plus className="mr-1 h-4 w-4" />빈 행
                </Button>
            ...
            {bulkOpen && (
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2 rounded-md border bg-muted/30 p-3">
                <Input
                  placeholder="세트 (예: OP01)"
                  value={(bulkPatch.set_code as string) ?? ""}
                  onChange={(e) => setBulkPatch((p) => ({ ...p, set_code: e.target.value }))}
                />
                <Select
                  value={(bulkPatch.game as string) ?? ""}
                  onValueChange={(v) => setBulkPatch((p) => ({ ...p, game: v as Game }))}
                >
                  <SelectTrigger>
                ...
                <Select
                  value={(bulkPatch.type as string) ?? ""}
                  onValueChange={(v) => setBulkPatch((p) => ({ ...p, type: v as CardType }))}
                >
                  <SelectTrigger>
                ...
                <Input
                  placeholder="레어도 (예: SR)"
                  value={(bulkPatch.rarity as string) ?? ""}
                  onChange={(e) => setBulkPatch((p) => ({ ...p, rarity: e.target.value }))}
                />
            ...
            {(errorRowsList.length > 0 || internalDups.size > 0) && (
              <div className="text-xs text-muted-foreground space-y-1 max-h-32 overflow-y-auto">
                {Array.from(internalDups.entries())
                  .slice(0, 5)
                  .map(([code, idxs]) => (
                    <div key={code}>
                      ...
                      {code} — {idxs.map((i) => i + 1).join(", ")}행
                    </div>
                  ))}
                {issuesByRow.map((iss, i) =>
                  iss
                ...
                <tbody>
                  {rows.map((r, i) => {
                    ...
                    const fieldErr = (f: string) =>
                      issues.some((x) => x.level === "error" && x.field === f);
                    const codeUpper = (r.code || "").toUpperCase();
                      ...
                      <tr key={i} className={`border-b border-border/40 ${rowBg}`}>
                        ...
                        <td className="px-2 py-1">
                          <div className="flex flex-col gap-1">
                            {r.image_url ? (
                              <img
                                src={normalizeImageUrl(r.image_url) ?? r.image_url}
                                alt=""
                            ...
                            <Input
                              ...
                              onBlur={(e) =>
                                updateRow(i, { image_url: normalizeImageUrl(e.target.value) })
                              }
                        ...
                        <td className="px-1 py-1">
                          ...
                          {(isInternalDup || isDbDup) && (
                            <Badge
                              variant={isInternalDup ? "destructive" : "secondary"}
                        ...
                        <td className="px-1 py-1">
                          ...
                          >
                            <SelectTrigger
                              className={`h-7 text-xs w-24 ${fieldErr("set_code") ? errCls : ""}`}
                            >
                            ...
                            <SelectContent>
                              {Array.from(
                                new Set([
                                  r.set_code,
                                  ...allSets.filter((s) => s.game === r.game).map((s) => s.name),
                                ]),
                              )
                                .filter(Boolean)
                        ...
                        <td className="px-1 py-1">
                          <Select
                            value={r.game}
                            onValueChange={(v) => updateRow(i, { game: v as Game })}
                          >
                        ...
                        <td className="px-1 py-1">
                          <Input
                            ...
                            className={`h-7 text-xs min-w-[140px] ${fieldErr("name") ? errCls : ""}`}
                          />
                        </td>
                        <td className="px-1 py-1">
                          <Select
                            value={r.type}
                            onValueChange={(v) => updateRow(i, { type: v as CardType })}
                          >
                            <SelectTrigger className="h-7 text-xs w-24">
                              <SelectValue />
                            </SelectTrigger>
                        ...
                        <td className="px-1 py-1">
                          <Input
                            ...
                            className={`h-7 text-xs w-24 ${fieldErr("colors") ? errCls : ""}`}
                          />
                        </td>
                        <td className="px-1 py-1">
                          <Input
                            value={r.cost ?? ""}
                            onChange={(e) => updateRow(i, { cost: num(e.target.value) })}
                        ...
                        <td className="px-1 py-1">
                          <Input
                            value={r.power ?? ""}
                            onChange={(e) => updateRow(i, { power: num(e.target.value) })}
                            className="h-7 text-xs"
                          />
                        </td>
                        <td className="px-1 py-1">
                          <Input
                            value={r.counter ?? ""}
                            onChange={(e) => updateRow(i, { counter: num(e.target.value) })}
                            className="h-7 text-xs"
                          />
                        </td>
                        <td className="px-1 py-1">
                          <Input
                            value={r.rarity ?? ""}
                            onChange={(e) => updateRow(i, { rarity: e.target.value || null })}
                            className={`h-7 text-xs w-16 ${fieldErr("rarity") ? errCls : ""}`}
                          />
                        </td>
                          ...
                          {r.game === "dtcg" && r.extra
                            ? (() => {
                                ...
                                const parts = [
                                  e.category,
                                  e.form,
                                  e.evo_cost_1 && `진화1:${e.evo_cost_1}`,
                                  e.evo_cost_2 && `진화2:${e.evo_cost_2}`,
                                ].filter(Boolean) as string[];
                                return parts.length ? (
                                  <span
                                    title={parts.join(" · ")}
                                    className="line-clamp-2 max-w-[120px]"
                                  >
                                    {parts.join(" · ")}
                                  </span>
                                ) : (
      ...
      {rows.length > 0 && (
        <div className="sticky bottom-4 flex items-center justify-between rounded-lg border bg-card/95 backdrop-blur p-3 shadow-lg">
          <div className="text-sm">
            ...
            {rows.length - valid.length > 0 && (
              <span className="text-destructive">
                {" "}
                · {rows.length - valid.length}건 필수 항목 누락
              </span>
            )}
          </div>
          {busy && progress.total > 0 && (
          ...
          <Button onClick={submit} disabled={busy || valid.length === 0}>
            ...
            {busy
              ? `등록 중… ${Math.round((progress.done / Math.max(progress.total, 1)) * 100)}%`
              : `${valid.length}건 등록`}
          </Button>
        </div>
      )}
      ...
      <ImageEditDialog
        ...
        onSaved={(url) => {
          if (editIdx !== null) updateRow(editIdx, { image_url: url });
        }}
      />
    </div>
  );
...
function SingleForm({ onAdd }: { onAdd: (r: CardRow) => void }) {
  ...
  const { games, labelOf } = useGames();
  const displaySets = Array.from(new Set([r.set_code, ...sets]))
    .filter(Boolean)
    .sort((a, b) => a.localeCompare(b));
        ...
        setR((prev) => {
          if (!prev.image_url) {
            return {
              ...
              extra_images: [...(prev.extra_images ?? []), ...uploaded.slice(1)],
            };
          }
  ...
  const setExtra = (k: string, v: string) =>
    setR((prev) => ({
      ...prev,
      extra: { ...((prev.extra as Record<string, unknown>) ?? {}), [k]: v },
    }));

  const submit = () => {
    if (!r.code || !r.set_code || !r.name) {
      toast.error("코드, 세트, 이름은 필수입니다");
      return;
    }
    ...
    const parsedColors = colorsInput
      ...
      .map((s) => s.trim())
      .filter(Boolean);
    const parsedTraits = traitsInput
      .split(/[|,;/]/)
      .map((s) => s.trim())
      .filter(Boolean);
    ...
    if (r.game === "dtcg") {
      ...
      out.counter = null;
      if (e.category && DIGIMON_CATEGORY_TYPE[e.category])
        out.type = DIGIMON_CATEGORY_TYPE[e.category];
    ...
    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
      ...
      <div className="md:col-span-2 space-y-1.5">
        ...
        <Input
          ...
          placeholder={r.game === "dtcg" ? "엘리자몬" : "몽키 D 루피"}
        />
      </div>
      ...
      {r.game === "dtcg" ? (
        <>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              value={r.rarity ?? ""}
              onChange={(e) => setR({ ...r, rarity: e.target.value || null })}
              placeholder="R / SR / SEC"
            />
          </div>
          <div className="space-y-1.5">
            <Label>DP</Label>
            <Input
              type="number"
              value={r.power ?? ""}
              onChange={(e) => setR({ ...r, power: num(e.target.value) })}
              placeholder="5000"
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              type="number"
              value={r.cost ?? ""}
              onChange={(e) => setR({ ...r, cost: num(e.target.value) })}
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              value={ex.evo_cost_1 ?? ""}
              onChange={(e) => setExtra("evo_cost_1", e.target.value)}
              placeholder="예: Lv.3"
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              value={ex.evo_cost_2 ?? ""}
              onChange={(e) => setExtra("evo_cost_2", e.target.value)}
              placeholder="예: Lv.4 / -"
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              value={r.attribute ?? ""}
              onChange={(e) => setR({ ...r, attribute: e.target.value || null })}
              placeholder="백신종 / 데이터종 / 바이러스종"
            />
          </div>
          <div className="space-y-1.5">
            <Label>
              유형 <span className="text-xs text-muted-foreground">(쉼표/| 구분)</span>
            </Label>
            <Input
              value={traitsInput}
              onChange={(e) => setTraitsInput(e.target.value)}
              placeholder="리버레이터, 파충류형"
            />
          </div>
          <div className="md:col-span-2 space-y-1.5">
            <Label>상단 텍스트</Label>
            <Textarea
              value={ex.text_top ?? ""}
              onChange={(e) => setExtra("text_top", e.target.value)}
              rows={2}
              placeholder="[등장 시] ..."
            />
          </div>
          <div className="md:col-span-2 space-y-1.5">
            <Label>하단 텍스트</Label>
            <Textarea
              value={ex.text_bottom ?? ""}
              onChange={(e) => setExtra("text_bottom", e.target.value)}
              rows={2}
              placeholder="[자신의 턴] ..."
            />
          </div>
        ...
        <>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              type="number"
              value={r.counter ?? ""}
              onChange={(e) => setR({ ...r, counter: num(e.target.value) })}
            />
          </div>
          <div className="space-y-1.5">
            <Label>속성</Label>
          ...
          <div className="md:col-span-2 space-y-1.5">
            ...
            <Textarea
              value={r.effect ?? ""}
              onChange={(e) => setR({ ...r, effect: e.target.value || null })}
              rows={3}
            />
          </div>
        </>
      ...
      <div className="md:col-span-2 space-y-2 rounded-md border border-dashed p-3">
        ...
        <p className="text-[11px] text-muted-foreground">
          첫 번째 이미지가 <b>메인 카드</b>로 카드 DB 상세에 표시됩니다. 순서 변경·삭제는{" "}
          <b>이미지 등록</b> 팝업에서 가능합니다.
        </p>
