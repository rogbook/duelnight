...
const SNIPPETS: { label: string; effect: Record<string, unknown> }[] = [
  { label: "드로우", effect: { id: "draw1", label: "등장 시 1장 드로우", trigger: "on_play", actions: [{ kind: "draw", count: 1 }] } },
  { label: "상대 KO", effect: { id: "ko1", label: "등장 시 상대 코스트3 이하 1장 KO", trigger: "on_play", actions: [{ kind: "ko_target", count: 1, target: { selector: "opponent_character_filter", filter: { cost_max: 3 } } }] } },
  { label: "파워 +1000", effect: { id: "pow1", label: "등장 시 이 캐릭터 +1000", trigger: "on_play", actions: [{ kind: "power_modifier", delta: 1000, duration: "this_turn", target: { selector: "self_active" }, scope: "single" }] } },
...
const ACTIONS = "draw · discard_hand · look_deck · search_deck · ko_target · return_to_hand · rest_target · active_target · power_modifier · attach_don · return_don_to_deck · gain_keyword · look_life · add_to_life · modify_damage · choose_one";
...
function validate(text: string): { ok: boolean; data?: unknown[]; error?: string } {
  ...
  if (!res.success) {
    ...
    return { ok: false, error: `스키마 오류: ${first.path.join(".") || "(root)"} — ${first.message}` };
  }
...
export function CardEffectsEditor({ cardId, initial }: { cardId: string; initial?: unknown }) {
  ...
  const addSnippet = (effect: Record<string, unknown>) => {
    ...
    const uniq = { ...effect, id: `${String(effect.id)}_${Math.random().toString(36).slice(2, 6)}` };
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className={`text-[11px] ${v.ok ? "text-emerald-500" : "text-destructive"}`}>
          {v.ok ? (
            <span className="inline-flex items-center gap-1"><Check className="h-3 w-3" />유효 · 효과 {count}개</span>
          ) : (
            <span className="inline-flex items-center gap-1"><AlertTriangle className="h-3 w-3" />{v.error}</span>
          )}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {SNIPPETS.map((s) => (
          <Button key={s.label} type="button" variant="outline" size="sm" className="h-7 text-[11px]" onClick={() => addSnippet(s.effect)}>
      ...
      <details className="rounded-md border border-border bg-muted/30 p-2 text-[11px] text-muted-foreground">
        <summary className="cursor-pointer select-none font-medium text-foreground">작성 도움말 (트리거·액션 목록)</summary>
        <div className="mt-2 space-y-1">
          <p><b>trigger</b>: {TRIGGERS}</p>
          <p><b>action.kind</b>: {ACTIONS}</p>
          <p className="text-muted-foreground/80">각 효과는 <code>{`{ id, trigger, actions:[...] }`}</code> 형태. 자세한 파라미터는 docs/SIMULATOR_SPEC.md 참고. 저장은 스키마 검증을 통과해야만 됩니다.</p>
        </div>
      </details>
