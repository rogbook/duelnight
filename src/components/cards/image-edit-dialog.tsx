...
/** 카드 이미지 회전·크롭 후 저장. card-images 버킷에 새 WebP 업로드 → publicUrl 반환. */
export function ImageEditDialog({ open, imageUrl, setCode, cardCode, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (open) { setRotation(0); setCrop(null); }
  }, [open, imageUrl]);
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => setRotation(r => ((r + 270) % 360) as 0 | 90 | 180 | 270)}>
            <RotateCcw className="h-4 w-4 mr-1" />좌 90°
          </Button>
          <Button variant="outline" size="sm" onClick={() => setRotation(r => ((r + 90) % 360) as 0 | 90 | 180 | 270)}>
        ...
        <div className="flex justify-center bg-muted/30 rounded-md p-4">
          <div
            ref={wrapRef}
            className="relative inline-block select-none cursor-crosshair max-h-[60vh]"
          ...
          >
            {imageUrl && (
              <img
                ...
                alt=""
                draggable={false}
                className="max-h-[60vh] block pointer-events-none"
        ...
        <DialogFooter>
          ...
          <Button onClick={save} disabled={busy || !imageUrl}>
