...
export const checkIsAdmin = createServerFn({ method: "POST" }).handler(async () => {
...
export const grantAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: targetId, error } = await supabase.rpc("grant_admin_by_email", { _email: email });
    if (error) throw new Response(error.message, { status: 400 });
...
export const revokeAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: targetId, error } = await supabase.rpc("revoke_admin_by_email", {
      _email: email,
    });
    if (error) throw new Response(error.message, { status: 400 });
    return { userId: targetId as string };
...
export const grantAiUnlimited = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: targetId, error } = await supabase.rpc("grant_ai_unlimited_by_email", {
      _email: email,
    });
    if (error) throw new Response(error.message, { status: 400 });
    return { userId: targetId as string };
...
export const revokeAiUnlimited = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { supabase, userId } = await getAuthContext();
    await assertAdmin(supabase, userId);
    const { data: targetId, error } = await supabase.rpc("revoke_ai_unlimited_by_email", {
      _email: email,
    });
    if (error) throw new Response(error.message, { status: 400 });
    return { userId: targetId as string };
...
/** AI 무제한 사용자 목록. */
export const listAiUnlimited = createServerFn({ method: "POST" }).handler(async () => {
  const { supabase, userId } = await getAuthContext();
  await assertAdmin(supabase, userId);
...
/** 관리자 존재 여부. */
export const anyAdminExists = createServerFn({ method: "POST" }).handler(async () => {
