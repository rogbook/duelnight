    ...
    <div className="mx-auto flex h-[calc(100dvh-3.5rem)] w-full max-w-2xl flex-col px-0 sm:px-4">
            ...
            return (
              <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                ...
                >
                  ...
                  <p
                    className={`mt-0.5 text-right text-[10px] ${mine ? "text-primary-foreground/70" : "text-muted-foreground"}`}
                  >
      ...
      {blocked ? (
        <div className="border-t border-border bg-muted/40 px-4 py-3 text-center text-xs text-muted-foreground">
          {t("dm.blockedNotice")}{" "}
          <button onClick={onUnblock} className="font-semibold text-primary underline">
            {t("dm.unblock")}
          </button>
        </div>
      ) : (
