...
function normalizeRow(obj: Record<string, string>): CardRow | { error: string } {
  ...
  const upload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < parsed.rows.length; i += CHUNK) {
        ...
        if (isAdmin) {
          // Admins can overwrite existing cards
          const { error } = await supabase.from("cards").upsert(slice, { onConflict: "code" });
          if (error) throw error;
          inserted += slice.length;
        } else {
          // Regular users: insert-only, ignore duplicates so existing cards aren't overwritten
      ...
      toast.success(
        skipped > 0
          ? `${inserted}장 추가 · ${skipped}장은 이미 존재해 건너뜀`
          : `${inserted}장 등록 완료`,
      );
    } catch (e) {
  ...
  if (loading) {
    return (
      <div className="mx-auto w-full max-w-4xl px-6 py-8">
        <PageHeader title="카드 대량 등록" description="확인 중…" />
      </div>
    );
  }
  if (!user) {
    return (
      <div className="mx-auto w-full max-w-4xl px-6 py-8">
        <PageHeader title="카드 대량 등록" description="로그인이 필요합니다" />
        <Card className="mt-6">
          <CardHeader>
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-6 py-8">
      <PageHeader
        ...
        description={
          isAdmin
            ? "CSV 또는 JSON으로 카드 데이터를 대량 등록·갱신합니다 (관리자 모드: code 기준 upsert)"
            : "CSV 또는 JSON으로 카드 데이터를 대량 등록합니다. 이미 등록된 카드(code 중복)는 건너뜁니다."
        }
      />
